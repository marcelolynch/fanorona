...
static void rellenarTablero(TableroADT  tablero);
static void liberarTodo(tCasilla ** matriz, int n);
static void limpiarTocadas(TableroADT  tablero);
static void actualizarTablero(TableroADT  tablero, tMovimiento * mov);
static void incrementoSegunDir(int * dirFil, int *dirCol, enum tDireccion direccion);
static int aumentarPosibles(TableroADT  tablero, tVecMovs * movsPosibles, tCoordenada casillaOrig);
static enum tDireccion direccionDestino(tCoordenada origen, tCoordenada destino);
static int meCapturan(TableroADT tablero, tCoordenada posicion, int jugador);
static enum tCaptura hayComida (int jugador, TableroADT tablero, tCoordenada origen, enum tDireccion direccion);
static int validarMovimiento(int jugador, TableroADT  tablero, tMovimiento *movimiento);
static int jugadaObligada(TableroADT  tablero, int jugador, tCoordenada origen);
static void copiarTablero(TableroADT  tablero);
static void intercambiarTableros(TableroADT  tablero);
...
*/
tFlag puedeEncadenar(TableroADT  tablero){
	return tablero->hayCadena;
}

int numColumnas(TableroADT  tablero){
	return tablero->cols;
}

int numFilas(TableroADT  tablero){
	return tablero->filas;
}

int consultarOcupante(TableroADT  tablero, int f, int c){
	return tablero->matriz[f][c].ocupante;
}

int consultarTipo(TableroADT  tablero, int f, int c){
	return tablero->matriz[f][c].tipo;
}

int mover (char jugador, int modo, TableroADT  tablero,tMovimiento * movimiento) {
	int captura;
	...
	if (captura == AMBOS) { /* si la jugada es ambigua, retorna para que el front pida la captura */
		...
		if (!(tablero->hayCadena) && modo == PVE && jugador == BLANCO)
		...
		if (captura != NINGUNO) /* si no fue PAIKA, busca una posible cadena */
			tablero->hayCadena = jugadaObligada(tablero, jugador, movimiento->coordDest);

		if (tablero->hayCadena) {
...
int estadoJuego(TableroADT  tablero, int jugador){
...
void cambiarTurno (int *jugador, TableroADT  tablero) {
	*jugador = !(*jugador);
	limpiarTocadas(tablero);
	tablero->direccionPrevia = NULA; /* Ninguna*/
	tablero->primerUndo=1;
}


int undo(TableroADT  tablero){
	int estado;

	if(tablero->primerUndo){
		intercambiarTableros(tablero);
		tablero->primerUndo=0;
		estado = OK;
	}
...
int calcularMovCompu(tMovimiento * mov, TableroADT  tablero){
...
int guardarPartida(TableroADT  tablero, int modo, int jugador, const char * nombre){
	FILE *f;
...
TableroADT  cargarPartida(int * modo, int * jugador, const char * nombre){
	/*asumo que los datos estan validados, el archivo no esta corrputo*/
	int fils,cols;
	int i,j;
	FILE * f;
	TableroADT  tablero = malloc(sizeof(*tablero));
	if(tablero==NULL)
		return NULL;
...
TableroADT  nuevoTablero(int fils, int cols, int modo){
	TableroADT  nuevo;
	nuevo=malloc(sizeof(*nuevo));	
	if(nuevo!=NULL){
		*nuevo = generarTablero(fils, cols, modo);
		nuevo->hayPaika=0;
		nuevo->hayCadena=0;
		nuevo->primerUndo=0;
		nuevo->direccionPrevia=NULA;
	}
...
TableroCDT generarTablero(int fils, int cols, int modo){
...
int generarAuxiliar(TableroADT  tablero){
...
void intercambiarTableros(TableroADT  tablero){
...
static void actualizarTablero(TableroADT  tablero, tMovimiento * mov){
	...
	enum tDireccion direccion = tablero->direccionPrevia;
...
static int aumentarPosibles(TableroADT  tablero, tVecMovs * movsPosibles, tCoordenada casillaOrig){
	...
	for(n=0; n<dirsPosibles ; n++){
		...
		if(dir != tablero->direccionPrevia){
			incrementoSegunDir(&dirFil, &dirCol, dir);
			#define FDERANGO(x,y) ((x) < 0 || (y) < 0 || (x) >= tablero->filas || (y) >= tablero->cols)             
			...
			if(!FDERANGO(casillaOrig.fil + dirFil, casillaOrig.col + dirCol) && tablero->matriz[casillaOrig.fil + dirFil][casillaOrig.col + dirCol].estado != TOCADA ){
				if( (tipoComida = hayComida(NEGRO, tablero, casillaOrig, dir)) != NINGUNO || tablero->hayPaika ){
...
/*	int hayPaika; */
	...
	tablero->direccionPrevia = direccionMov;
...
static int jugadaObligada(TableroADT  tablero, int jugador, tCoordenada origen){
	...
	for(dir=0 ; dir<dirsPosibles ; dir++){
		...
		if(tablero->matriz[destino.fil][destino.col].estado != TOCADA && direcciones[dir] != tablero->direccionPrevia)
