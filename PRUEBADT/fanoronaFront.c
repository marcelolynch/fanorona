...
int jugar(TableroADT tablero, int modo, int jugador);
void pedirDimensiones(int *fils, int *cols);
...
void imprimirTablero ( TableroADT  tablero );
...
int main(void){
	...
	if(opcion == PVP || opcion == PVE){
		...
		tablero=nuevoTablero(filas, columnas, modo);
		...
		if((tablero = cargarPartida(&modo, &jugador, nombre))== NULL){ /*modo cambia al correspondiente (0 o 1)*/
		...
		printf("DIMENSIONES DEL TABLERO: %dx%d\n\n",numFilas(tablero),numColumnas(tablero));
	}
...
int jugar(TableroADT tablero, int modo, int jugador){
	...
	while (!hayGanador && jugada != QUIT) {
		...
		if (!hayGanador) {
			...
			if (puedeEncadenar(tablero) && (jugador == BLANCO || modo == PVP)) /* si hay cadena y no es la computadora */
				pedirCadena(&mov);
			...
			if (jugada == MOV) {
				...
				if (captura >= 0) { /* si el movimiento fue válido */
					imprimirTablero(tablero);
					if (!puedeEncadenar(tablero)) { /* cambiamos de turno */
...
void pedirDimensiones(int *f, int *c){	
	tFlag hayError;
	int decision;
...
void imprimirTablero ( TableroADT  tablero ){
	...
	for(i=-1; i<numFilas(tablero); i++)
		{	putchar('\n');
			if(i<0)
				printf("\t\t\t");
			else
				printf("\t\t\t%d\t",i<0?' ':i+1);
			for(j=0; j<numColumnas(tablero); j++)
			{	if(i==-1)
				{	if(j==0)
						putchar('\t');
					printf("%-4d",j+1);

					if( j== numColumnas(tablero)-1)
						putchar('\n');
				}
				else
				{	if (consultarTipo(tablero,i,j)==DEBIL)
						printf("%-4c", tolower( idColor[ consultarOcupante(tablero,i,j) ] )); /*BLANCO=0, NEGRO=1, VACIO=2*/
					else 
						printf("%-4c", idColor[ consultarOcupante(tablero,i,j) ] );
				}
			}
		}
		putchar('\n');
